...
import { instantiateWebRtcObject } from './utils/instantiateWebRTC';
import ClientSocketIO from './utils/ClientSocketIO';

$.getScript('../js/lib/jqcloud.min.js', (data, status, jqxhr) => {
  if (status && status === 'success') {
    console.log('jQCloud loaded successfully');
  }
  else {
    console.log('jQCloud could not load ', status);
  }
});
...
const initialLang = getValidLanguage('en_US');
  ...
  fetchConfig().done((data) => {
    ...
    onboardIfNeeded().done(() => {
      fetchStartupData().done(() => {
        ...
        // instantiate WebRTC obejct
        app.webRtc = instantiateWebRtcObject('http://45.76.95.99:8888/');

        // Used for initialization of our custom SocketIO object, which will be used
        // for listening and emitting three events(for now) - call, accept and decline
        app.io = new ClientSocketIO({
          url: 'http://45.76.95.99:8888/',
          webRtc: app.webRtc,
          currentPeerId: app.profile.get('peerID'),
          currentPeerName: app.profile.get('name'),
