...
import { updateProfile } from '../../../utils/SearchEngineRequests';
    ...
    if (save) {
      ...
      $.whenAll(save, saveAvatar, saveHeader)
        .done(() => {
          ...
          this.updatedProfileData.id = app.profile.id;
          this.updatedProfileData.avatarHashes = app.profile.get('avatarHashes').attributes;
          this.updatedProfileData.name = app.profile.get('name');
          updateProfile(this.updatedProfileData);
        })
        .fail((args) => {
          const errMsg =
