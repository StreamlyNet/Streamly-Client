...
export default class extends baseVw {
  ...
  onChatHeadClick(e) {
    ...
    this._openConversation(e.view.model.id, profilePromise, e.view.profile.get('name'));
  }
   ...
   */
  _openConversation(guid, profilePromise, name) {
    this.open();
  ...
  openConversation(options, profile) {
    if (!options.targetID) {
      throw new Error('Please provide a guid.');
    }
    ...
    if (profile) {
      ...
      this.profileDeferreds[options.targetID] = deferred;
      deferred.resolve(profile);
      profilePromise = deferred.promise();
    } else {
      // If not providing the profile, we'll fetch it.
      profilePromise = this.fetchProfiles([options.targetID])[0];
    }

    this._openConversation(options.targetID, profilePromise, options.name);
  }
