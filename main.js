...
const serverPath = `${__dirname}${path.sep}..${path.sep}streamly-go${path.sep}`;
const serverFilename = process.platform === 'darwin' || process.platform === 'linux' ?
  'streamlyd' : 'streamlyd.exe';
...
switch (process.platform) {
  case 'win32':
    defaultUserDataPath = `${homedir()}\\Streamly1.0-ClientData`;
    break;
  case 'darwin':
    defaultUserDataPath = `${homedir()}/Library/Application Support/Streamly1.0-ClientData`;
    break;
  default:
    defaultUserDataPath = `${homedir()}/.streamly1.0-clientData`;
}
...
function createWindow() {
  ...
  const osTrayIcon = 'streamlyVectorIconNoText.png';
